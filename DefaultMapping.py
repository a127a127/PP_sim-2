    ...
    def map(self):
        ...
        for nlayer in range(self.layer_length):
            if self.model_info.layer_list[nlayer].layer_type == "convolution":
                ...
                xbar_mapping_idx -= used_xbar_num
                l = len(inputs) // used_pe_num # split into multiple pe
...
class TransferMapping_o(object):
    def __init__(self, hd_info, model_info):
        self.model_info = model_info
        self.hd_info = hd_info
    ...
    def map(self):
        ...
        for nlayer in range(self.layer_length):
            if self.model_info.layer_list[nlayer].layer_type == "convolution":
                ...
                num_XB_h = ceil(matrix_height / self.hd_info.Xbar_h) # number of XB per column
                num_XB_w = ceil(matrix_width / self.hd_info.Xbar_w) # number of XB per row

                

                for xb_w_idx in range(num_XB_w):
                    for xb_h_idx in range(num_XB_h): # traverse height first
                        # XB
                    ...
                    if remain_xb_in_pe < num_XB_h: # change PE
