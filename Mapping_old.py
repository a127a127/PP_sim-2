    ...
    def map(self):
        ...
        for nlayer in range(self.model_info.layer_length):
            if self.model_info.layer_list[nlayer].layer_type == "convolution":
                ...
                pe_y_num = ceil(matrix_height / ((self.hd_info.Xbar_h // 2) * self.hd_info.Xbar_num_y * self.hd_info.CU_num_y))      
                pe_x_num = ceil(matrix_width / ((self.hd_info.Xbar_w // 2) * self.hd_info.Xbar_num_x * self.hd_info.CU_num_x))
                pe_total_num = pe_y_num * pe_x_num # used PE num

                for w in range(matrix_width):
                    for h in range(matrix_height):
                        # traverse all cells
                        cell_h = h % (self.hd_info.Xbar_h // 2) # TODO: 不整除的情況要考慮
                        cell_w = w % (self.hd_info.Xbar_w // 2)
                        xb_h = h // (self.hd_info.Xbar_h // 2) % self.hd_info.Xbar_num_y
                        xb_w = w // (self.hd_info.Xbar_w // 2) % self.hd_info.Xbar_num_x
                        cu_h = h // ((self.hd_info.Xbar_h // 2) * self.hd_info.Xbar_num_y) % self.hd_info.CU_num_y
                        cu_w = w // ((self.hd_info.Xbar_w // 2) * self.hd_info.Xbar_num_x) % self.hd_info.CU_num_x
                        pe_h = h // ((self.hd_info.Xbar_h // 2) * self.hd_info.Xbar_num_y * self.hd_info.CU_num_y)
                        pe_w = w // ((self.hd_info.Xbar_w // 2) * self.hd_info.Xbar_num_x * self.hd_info.CU_num_x)                        
                        ...
                        print(rt_h, rt_w, pe_h, pe_w, cu_h, cu_w, xb_h, xb_w, cell_h, cell_w)
                        self.crossbar_array[rt_h][rt_w][pe_h][pe_w][cu_h][cu_w][xb_h][xb_w][cell_h][cell_w] = [nlayer, ngrid, nfilter, nbit]
                ...
                xb_y_num = ceil(matrix_height / (self.hd_info.Xbar_h // 2))
                xb_x_num = ceil(matrix_width  / (self.hd_info.Xbar_w // 2))
                ...
                for w in range(xb_x_num):
                    for h in range(xb_y_num):
                        ...
                        xbar_inputs = inputs[:, h*(self.hd_info.Xbar_h // 2) : (h+1)*(self.hd_info.Xbar_h // 2)].tolist()

                        if (w+1) * (self.hd_info.Xbar_w // 2) <= matrix_width:
                            xbar_column = [i for i in range(self.hd_info.Xbar_w // 2)]
                        else:
                            empt = (w+1) * (self.hd_info.Xbar_w // 2) - matrix_width
                            full = (self.hd_info.Xbar_w // 2) - empt
                            xbar_column = [i for i in range(full)]
                        
                        if (h+1) * (self.hd_info.Xbar_h // 2) <= matrix_height:
                            xbar_row = [i for i in range(self.hd_info.Xbar_h // 2)] 
                        else:
                            empt = (h+1) * (self.hd_info.Xbar_h // 2) - matrix_height
                            full = self.hd_info.Xbar_h // 2 - empt
                            xbar_row = [i for i in range(full)]
